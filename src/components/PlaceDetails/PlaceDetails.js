    ...
    <Card elevation={6}>
      ...
      <CardContent >
          ...
          <Box display='flex' justifyContent='space-between'>
            <Rating value={Number(place.rating)} read-only></Rating>
        ...
        <Box display='flex' justifyContent='space-between'>
          <Typography variant='subtitle-1'>Rank</Typography>
           <Typography gutterBottom variant='subtitle-1'>{place.ranking}</Typography>
          </Box>
          {place?.awards?.map((award) => {
            return (
              <Box display='flex' justifyContent='space-between'>
                <img my={1} src={award.images.small} alt={award.display_name}></img>
          ...
          {place?.address && (
            <Typography gutterBottom varinat='subtitle2' color='text-secondary' className={classes.subtitle}>
              <LocationOnIcon/>{place.address}
            </Typography>
            )
          }
          {place?.phone && (
            <Typography gutterBottom varinat='subtitle2' color='text-secondary' className={classes.spacing}>
              <Phone/>{place.phone}
            </Typography>
            )
          }
          <CardActions>
            <Button size='small' color='secondary' onClick={() => window.open(place.web_url, '_blank')}>Travel Advisor</Button>
